...
contract('Factory', function(){
    ...
    const approvedAmount2 = BigNumber.from(1).mul(BigNumber.from(10).pow(BigNumber.from(daiDecimals - 2, 10)));  //1 DAI
    ...
    it('should swap token1 to dai', async () => {
        ...
        console.log(`amountOutMin Type : ` + typeof(amountOutMin));
        console.log(`amountOutMin : ` + amountOutMin );
        const path = [Token1Address, DAIAdrress];
        const deadline = Math.floor(Date.now() / 1000) + 60*20;
        let to = clientAddress;
        await token1.approve(Router02Address, amountIn, {from : clientAddress});
